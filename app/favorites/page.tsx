  ...
  return (
    <div className="space-y-8">
      <header className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
        <div>
          <h1 className="text-3xl font-semibold text-[var(--c-dark)]">Saved properties</h1>
      ...
      {!savedProperties.length && (
        <div className="rounded-xl border border-dashed border-gray-300 bg-white p-10 text-center text-gray-600">
          You have not saved any properties yet. Explore listings and select "Save" to build your shortlist.
        </div>
      )}
          ...
          {comparePair.map((property) => (
            <article key={property.id} className="card space-y-4">
              <header className="flex items-start justify-between gap-3">
                <div>
                  <h2 className="text-xl font-semibold text-[var(--c-dark)]">{property.title}</h2>
                  <p className="text-sm text-gray-600">
                    {property.city} - ${property.rent}/mo
                  </p>
                </div>
                <button type="button" className="btn" onClick={() => toggleFavorite(property.id)}>
                  Remove
                </button>
              </header>
